...
const RANGE = { fromIso: "2026-06-01T00:00:00.000Z", toIso: "2026-06-30T23:59:59.999Z" };
...
function makeWindsorSource(overrides: Partial<WindsorSourceSummary> = {}): WindsorSourceSummary {
  return {
    source: "facebook",
    spend: 1000,
    impressions: 50000,
...
describe("aggregateEfforts", () => {
  it("returns all zeros and empty arrays when given null windsor and no reports", () => {
    ...
    expect(result.totals).toEqual({ spend: 0, impressions: 0, reach: 0, clicks: 0 });
  ...
  it("sums manual fields across multiple reports from multiple branches", () => {
    const reports = [
      ...
      makeReport({
        ...
        emailsSent: 2,
        postsPublished: 1,
        contacts: [{ channel: "whatsapp", count: 10 }, { channel: "llamada", count: 2 }],
      }),
    ];
  ...
  it("counts distinct branches and days correctly across multiple reports", () => {
    const reports = [
      makeReport({ reportId: "centro_2026-06-10", branch: "centro", date: "2026-06-10" }),
      makeReport({ reportId: "norte_2026-06-10", branch: "norte", date: "2026-06-10" }),
      makeReport({ reportId: "centro_2026-06-11", branch: "centro", date: "2026-06-11" }),
    ];
    const result = aggregateEfforts(null, reports, RANGE);
    expect(result.manual.reportingBranches).toBe(2);
  ...
  it("maps windsor bySource to platforms correctly", () => {
    const windsor = {
      bySource: [
        makeWindsorSource({ source: "facebook", spend: 2000, impressions: 100000, reach: 60000, clicks: 1200 }),
        makeWindsorSource({ source: "google", spend: 1500, impressions: 80000, reach: 50000, clicks: 900 }),
      ],
    ...
    expect(result.platforms[0]).toEqual({ source: "facebook", spend: 2000, impressions: 100000, reach: 60000, clicks: 1200 });
    expect(result.totals).toEqual({ spend: 3500, impressions: 180000, reach: 110000, clicks: 2100 });
  });
  ...
  it("preserves the range in the result", () => {
    const range = { fromIso: "2026-01-01T00:00:00.000Z", toIso: "2026-01-07T23:59:59.999Z" };
    const result = aggregateEfforts(null, [], range);
